jQuery( document ).ready( function( $ ) {
	...
	// version 1.0.4 with modification line #13 ~ #21
	/**
	 ...
	 */
	/**
	 * note: when visual composer is active, the following prevents VC from working with
	 * 		Uncaught TypeError: Cannot read property ‘getRelatedTab' of undefined)
	 */
	...
	function CMB2ConditionalsInit( changeContext, conditionContext ) {
		...
		changeContext.on( 'change', 'input, textarea, select', function( evt ) {
			...
			if( 'undefined' == typeof fieldName ) {
				fieldName = elm.attr( 'id' );
				// for textarea, etc.
				if('undefined' == typeof fieldName) {
					fieldName = elm.attr( 'data-parent-name' );
				}
			}
			...
			if ( dependants.length > 0 ) {
				...
				dependants.each( function( i, e ) {
					...
					// workaround
					if( 'undefined' == typeof currentFieldName ) {
						currentFieldName = current.attr( 'id' );
						// for textarea, etc.
						if('undefined' == typeof currentFieldName) {
							currentFieldName = current.attr( 'data-parent-name' );
						}
					}
		...
		var _dependency_elements_database = [];
		changeContext.find( '[data-conditional-id],[data-parent-name]').each( function( i, e ) {
			var _cond_data_attributes = _find_attributes($(e));
			var _should_hide = _is_hidden_recursive(_cond_data_attributes, false, 0);
			if(_should_hide === true) {
				$(e).parents( '.cmb-row:first' ).hide();
			}
		});


		function _is_hidden_recursive ( cond_data_attributes, should_hide, level ) {
			if(cond_data_attributes !== false) {
				/* set up variables */
				/* find element in question */
				var $_current_element = $('[name='+cond_data_attributes.cond_id +']',changeContext);
				var _this_value = null;
				if($_current_element.is('input:checkbox')) {
					/* checkbox */
					_this_value = $_current_element.prop("checked");
					if(_this_value === true) {
						_this_value = $_current_element.val();
					}
				} else if($_current_element.is('select')) {
					_this_value = $_current_element.find(':selected').val();
				} else {
				/* others */
					_this_value = $_current_element.val();
				}
				/* check up if the id name is found */
				if( cond_data_attributes.cond_values.indexOf(_this_value) === -1 ) {
					// console.log(cond_data_attributes.id, _this_value);
					/* Value not found. */
					should_hide = true;
				}
				level ++;
				should_hide = _is_hidden_recursive(_find_attributes($_current_element), should_hide, level);
			}

			/* End if undefined (not a dependent element) */
			return should_hide;
		}

		function _find_attributes ( $_this_element ) {
			var _this_id = $_this_element.attr('name');
			if(typeof _this_id === 'undefined') {
				_this_id = $_this_element.attr('data-parent-name');
			}
			var _conditional_id = $_this_element.attr('data-conditional-id');
			var _conditional_values = toJSON($_this_element.attr('data-conditional-value'));
			if(typeof _conditional_id === 'undefined') {
				// not conditional element but dependent
				return false;
			} else {
				// conditional element
				return {id:_this_id, cond_id: _conditional_id, cond_values: _conditional_values };
			}
		}
	...
	function CMB2ConditionalsRecursivelyHideDependants( fieldName, elm, context ) {
		...
		/**
		 * If the following line is active, somehow colorpicker plugin can not hidden.
		 */
	...
	CMB2ConditionalsInit( '[id^="tm_settings_"].postbox', '[id^="tm_settings_"].postbox .cmb2-wrap' );
});